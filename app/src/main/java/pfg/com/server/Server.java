...
public class Server {
    ...
    public void connect() {
        ...
        try {
            ...
            LogTool.logd(TAG, "socket已开启,等待连接");
            while (true) {
                socket = mServerSocket.accept();
                if (socket.isConnected()) {
                    LogTool.logd(TAG, "socket连接成功");
                    break;
                }
            }
        } catch (IOException e) {
            // TODO Auto-generated catch block
            LogTool.logd(TAG, "connect IOException:"+e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void disconnect() {
        ...
        try {
            if (socket != null && socket.isConnected()) {
                socket.close();
                mServerSocket.close();
                LogTool.logd(TAG, "socket断开了");
            }
        } catch (IOException e) {
            // TODO Auto-generated catch block
            LogTool.logd(TAG, "disconnect IOException:"+e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public byte[] readLength() {
        try {
            ...
            // TODO Auto-generated catch block
            LogTool.logd(TAG, "readLength IOException:"+e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
    ...
    public byte[] readSPSPPS(int length) {
        ...
        try {
            ...
            // TODO Auto-generated catch block
            e.printStackTrace();
            LogTool.logd(TAG, "readSPSPPS IOException:"+e.getMessage());
        }
        return null;
    }
    ...
    public Frame readFrame(int frameLength) {
        ...
        try {
            ...
            // TODO Auto-generated catch block
            e.printStackTrace();
            LogTool.logd(TAG, "readFrame IOException:"+e.getMessage());
        }
        return null;
    }
    ...
    public static byte[] readBytes(InputStream in, long length) {
        ...
        while (read < length) {
            ...
            try {
                ...
                // TODO Auto-generated catch block
                LogTool.logd(TAG, "readBytes IOException:"+e.getMessage());
                e.printStackTrace();
            }
            if (cur < 0) {
                break;
